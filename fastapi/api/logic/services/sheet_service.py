...
from api.shared.exceptions.sheet_exceptions import UserTableNotFoundException
...
class SheetService:
    ...
    def get_user_table(self, db_session: Session, id: int, user: UserBaseDto) -> TableDto:
        user_table_in_db: TableDao = self.__sheet_repo.get_user_table(
            db_session, id, user
        )

        if not user_table_in_db:
            raise UserTableNotFoundException

        return TableDto(
            id=user_table_in_db.id,
            name=user_table_in_db.name,
            columns=[ColumnDto(
                id=column.id,
                notation=column.notation,
                width=column.width,
            ...
            ) for column in user_table_in_db.columns],
            rows=[RowDto(
                id=row.id,
                number=row.number,
                height=row.height,
            ...
            ) for row in user_table_in_db.rows],
            cells=[CellDto(
                id=cell.id,
                content=cell.content,
                table_id=cell.table_id,
                row_id=cell.row_id,
                column_id=cell.column_id
            ) for cell in user_table_in_db.cells],
            user_id=user_table_in_db.user.id
        )
